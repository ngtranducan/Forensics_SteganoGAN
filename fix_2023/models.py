...
class SteganoGAN(object):
    ...
    def _make_payload(self, width, height, depth, text):
        ...
        """
        print("original:",text_to_bits(text))
        ...
        print("length cua payload: ", len(payload))
    ...
    def encode(self, cover, output, text):
        ...
        bit_string = ''.join([str(x) for x in payload.view(-1).tolist()])
        cover = cover.to(self.device)
    ...
    def decode(self, image):
        ...
        print("image.data.cpu()")
        print(image.data.cpu())
        
        print("image.data.cpu().numpy()")
        print((image.data.cpu().numpy() > 0).astype(int))

        
        
        
        image_tensor = torch.from_numpy(image.data.cpu().numpy())

        bits = (image_tensor.data.cpu().numpy() > 0).astype(int).tolist()  # apply the command to the image tensor

        print('length: ', len(bits))

        temp = bits_to_bytearray(bits)

        old_string = temp
        new_string = old_string.replace("0x", "x")

        

        for candidate in new_string.split('\\x0\\x0\\x0\\x0'):
            #print("first", candidate)
            candidate = bytearray_to_text(candidate)
            #print("---", candidate)
            if candidate:
